...
my @tuples = (Constructlist(Readapps('tuples_error.txt')));
...
my %tuples;
@tuples{@tuples} = ();
@file_list = grep !exists $tuples{$_}, @complete_filelist;
...
for ($j = 0; $j <= $#file_list; $j++)
{
    ...
    if($app eq "2"){
		...
		@bw_dist_1 = `grep "bw_dist" $file_list[$j] | tail -n 6 |  awk '\{print \$3\}'`;
		@bw_dist_2 = `grep "bw_dist" $file_list[$j] | head -n 6 |  awk '\{print \$4\}'`;
	
		$row_locality_1 = `grep "average row locality_1" $file_list[$j] |  tail -n 1 | awk '\{print \$7\}'`;
		$row_locality_2 = `grep "average row locality_2" $file_list[$j] |  head -n 1 | awk '\{print \$7\}'`;
    	...
    	$misses_2 = `grep "Stream 2: Misses" $file_list[$j] | head -n 1 | awk '\{print \$5\}'`;
        $misses_1 = `grep "Stream 1: Misses" $file_list[$j] | tail -n 1 | awk '\{print \$5\}'`;
		...
		@bw_dist_1 = `grep "bw_dist" $file_list[$j] | head -n 6 |  awk '\{print \$3\}'`;
		@bw_dist_2 = `grep "bw_dist" $file_list[$j] | tail -n 6 |  awk '\{print \$4\}'`;
	
		$row_locality_2 = `grep "average row locality_2" $file_list[$j] |  tail -n 1 | awk '\{print \$7\}'`;
		$row_locality_1 = `grep "average row locality_1" $file_list[$j] |  head -n 1 | awk '\{print \$7\}'`;
    	...
    	$misses_2 = `grep "Stream 2: Misses" $file_list[$j] | tail -n 1 | awk '\{print \$5\}'`;
        $misses_1 = `grep "Stream 1: Misses" $file_list[$j] | head -n 1 | awk '\{print \$5\}'`;
