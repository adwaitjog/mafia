...
do
              mkdir $benchmark
              cd $benchmark
              ln -s $benchmarks/nvkernels/gpgpu_ptx_sim__concurrentKernels .
              cp $applications/bash-scripts/run_scripts/nvkernels/mainscript_$benchmark .
...
if [ $1 = "--cleanup" ]; then
    ...
    for BMK in gups dgemm rtm hist gauss; do
